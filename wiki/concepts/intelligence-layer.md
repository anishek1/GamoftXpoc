...
**Open decision:** Primary provider — Groq (speed) vs OpenAI (reliability), or Groq primary with OpenAI fallback. `[TBD]`
...
1. **Single LLM call per lead.** Rating Agent makes exactly one logical model call.
...
| # | Decision | Options |
|---|---|---|
| 1 | Primary LLM provider | Groq (speed) primary + OpenAI fallback, or OpenAI primary + Groq overflow |
