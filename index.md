...
- [[wiki/analyses/orchestration-layer-spec]] — Subtask 3 responsibilities spec: two-pipeline architecture, LLM calls per lead (Sonnet always + Haiku on DM path), fill-in-the-blanks prompt, deterministic signal extraction, bucket thresholds 80/55/0; pipeline_stage locked values include awaiting_clarification; sub_scores locked to 5 dimensions (fit/intent/engagement/behavioral/context); two-path entry points; Consent Gate reference; enrichment fallback chain clarification | date: 2026-04-19 | updated: 2026-05-03 | status: COMPLETE
